...
Windsurf is a VS Code fork from Codeium, so the [Pinned VS Code extension](https://marketplace.visualstudio.com/items?itemName=pinnedai.pinnedai-vscode) already installs in it (via `windsurf --install-extension ...`). This page covers the AI-rules side — making Windsurf's AI respect Pinned guards.
...
3. Before telling the user a code change is complete, run `pinned review`.
4. Include the resulting PASS / REVIEW / BLOCK summary in the final response — do not hide it.
